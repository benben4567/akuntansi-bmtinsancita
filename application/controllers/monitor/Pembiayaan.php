...
function table_list_pembiayaan(){
    $date = date('Y-m-d');
    $query = $this->db->query("
        SELECT t1.*,t2.nama,t3.nama_pegawai,t4.grouppembiayaan_nama, t5.accounttrans_ref, count(t5.accounttrans_ref) as belum_lunas FROM tb_pembiayaan AS t1 
        INNER JOIN tb_nasabah AS t2 ON t2.nomor_nasabah                = t1.nomor_nasabah 
        INNER JOIN pegawai AS t3 ON t1.nomor_ao                        = t3.nip 
        INNER JOIN master_grouppembiayaan AS t4 ON t1.jenis_pembiayaan = t4.kode_produk 
        INNER JOIN tb_accounttrans AS t5 ON t1.nomor_rekening          = t5.accounttrans_user
        INNER JOIN tb_pembiayaandetail AS t6 ON t1.pembiayaan_id       = t6.id_pembiayaan
        WHERE t5.accounttrans_ref=0 AND T5.accounttrans_type='02'
        AND t6.tgl_angsuran
        ");
    $data = $query->result_array();

    $query = $this->db->query("
        SELECT t1.*, count(t5.accounttrans_ref) as lunas FROM tb_pembiayaan AS t1 
        INNER JOIN tb_accounttrans AS t5 ON t1.nomor_rekening=t5.accounttrans_user
        WHERE t5.accounttrans_ref=1
        ");
    $ref_o = $query->result_array();
