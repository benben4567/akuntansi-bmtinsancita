 ...
 $('#button-previous ,#button-next').click(function() {
    ...
    function domIsi() {
        ...
        $('.cedt').click( function() {
            ...
            $("#form_sample_1 input[name='nomor_npwp']").val(obj.nomor_npwp);
            $("#form_sample_1 input[name='nomor_bpjs']").val(obj.nomor_bpjs);
...
$(document).ready(function(){
    ...
    $('#get_filter_nasabah').click(function(){
        var jenis_tabungan = $('#jenis_tabungan').val();
        var start_date = $('#start_date').val();
        var end_date = $('#end_date').val();

        $.ajax({
            url: 'base/nasabah/get_filter_nasabah',
            type: 'post',
            async : false,
            dataType : 'json',
            data: {jenis_tabungan: jenis_tabungan,start_date: start_date, end_date: end_date},
            success: function(data){
                ...
                for(var i =0; i < data.length; i++){
                    isi += "<tr style=\"vertical-align:top;\">"
                    + "<td style=\"padding: 0 5px;\" align=\"center\">" + no + "</td>"
                    + "<td style=\"padding: 0 5px;\" align=\"left\">" + data[i].nomor_rekening + "</td>"
                    + "<td style=\"padding: 0 5px;\" align=\"left\">" + data[i].nama + "</td>"
                    + "<td style=\"padding: 0 5px;\" align=\"left\">" + data[i].jenis_tabungan + "</td>"
                    + "<td style=\"padding: 0 5px;\" align=\"left\">" + data[i].alamat + "</td>"
                    + "</tr>";
                    no++;
                };
                $('#filter_data_nasabah').html(isi);
            },
            error: function(data) {
    ...
    $('#print_filter_nasabah').on('click',function(){
        var divToPrint=document.getElementById('table_filter_nasabah');
